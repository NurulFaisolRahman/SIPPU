    ...
    <style>
        ...
        ::-webkit-scrollbar-track { background: #050e1d; border-radius: 4px; }
    ...
    <div class="w-full p-1 md:p-1 lg:p-1 pb-16">
        ...
        <div class="bg-gradient-to-r from-blue-900 via-indigo-950 to-[#0a1324] border border-blue-800/50 rounded-xl p-6 mb-6 flex flex-col sm:flex-row items-center justify-between shadow-lg relative overflow-hidden shrink-0">
            ...
            <form method="GET" action="<?= base_url('Admin/OpdInstansi') ?>" class="relative z-10 flex flex-wrap items-center gap-2 bg-[#050e1d]/80 p-3 rounded-xl border border-blue-700/40 backdrop-blur-sm">
                ...
                <!-- RENTANG BULAN CUSTOM (HIDDEN/SHOW) -->
                <div id="customBulanWrapper" class="flex items-center gap-2 <?= $selected_periode == 'custom' ? '' : 'hidden' ?>">
                    <div class="flex flex-col">
                        <label class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">Dari Bulan</label>
                    ...
                    <div class="flex flex-col">
                        <label class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">Sampai Bulan</label>
        ...
        <div class="admin-panel flex flex-col bg-[#0b172e] border border-[#1e2d4a] rounded-xl shadow-xl overflow-hidden">
            ...
            <div class="p-5 border-b border-[#1e2d4a] flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#081122]">
                <div class="text-xs font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                    ...
                    <span>Data Laporan: <strong class="text-emerald-400"><?= $selected_tahun ?></strong> (Bulan <?= $start_month ?> s/d <?= $end_month ?>)</span>
                </div>
                
                <!-- TOMBOL AKSI EXPORT & EDIT -->
            ...
            <div class="table-container overflow-x-auto w-full">
                <table class="w-full text-left text-[12px] text-slate-300 relative">
                    ...
                    <tbody class="divide-y divide-[#1e2d4a]/60">
    ...
    <script>
        ...
        // ==========================================
        // LOGIKA EXPORT EXCEL (EXCELJS) DINAMIS
        // ==========================================
        async function downloadExcel() {
            ...
            // Kolom Dinamis: NO, NAMA OPD, JENIS LAYANAN, [Bulan start..end], TOTAL
            ...
            for (let m = startMonth; m <= endMonth; m++) {
                headers.push(monthShortIndo[m - 1]);
            }
            headers.push('TOTAL');
        ...
        // ==========================================
        // LOGIKA EXPORT PDF (JSPDF) DINAMIS
        // ==========================================
        ...
        async function downloadPDF() {
            Swal.fire({
                title: 'Menyiapkan PDF Rekap MPP...', text: 'Mohon tunggu...',
            ...
            try {
                ...
                for (let m = startMonth; m <= endMonth; m++) {
                    tableHeaders.push(monthShortIndo[m - 1]);
                }
        ...
        // ==========================================
        // PAGINASI & HELPER INTERAKSI
        // ==========================================
        const itemsPerPage = 5;
        let currentPage = 1;
        const totalItems = rawDataOPD.length;
        ...
        function renderTablePage(page) {
            ...
            $('.parent-row, .child-row').each(function() {
                const idx = parseInt($(this).data('idx'));
                if (idx >= start && idx < end) {
                    if ($(this).hasClass('parent-row')) $(this).removeClass('page-hidden');
                } else {
                    $(this).addClass('page-hidden').addClass('hidden');
                }
            });

            $('#page-info-start').text(start + 1);
            $('#page-info-end').text(Math.min(end, totalItems));
        ...
        function buildPagination() {
            ...
            let html = `<button onclick="renderTablePage(${currentPage - 1})" ${currentPage === 1 ? 'disabled' : ''} class="px-2 py-1 rounded border border-[#1e2d4a] bg-[#050e1d] text-slate-400 disabled:opacity-50"><i data-lucide="chevron-left" class="w-4 h-4"></i></button>`;
            for(let i=1; i<=totalPages; i++) {
                const active = i === currentPage ? 'bg-blue-600 text-white' : 'bg-[#050e1d] text-slate-400';
                html += `<button onclick="renderTablePage(${i})" class="px-3 py-1 rounded border border-[#1e2d4a] ${active} text-xs">${i}</button>`;
            }
            html += `<button onclick="renderTablePage(${currentPage + 1})" ${currentPage === totalPages ? 'disabled' : ''} class="px-2 py-1 rounded border border-[#1e2d4a] bg-[#050e1d] text-slate-400 disabled:opacity-50"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>`;
            $('#pagination-controls').html(html);
            refreshIcons();
        }

        function refreshIcons() { if (typeof lucide !== 'undefined') lucide.createIcons(); }
        function toggleRow(id, el) { $(`#${id}`).toggleClass('hidden'); $(el).find('.icon-expand').toggleClass('rotate-90'); }
        function openModal(id) { $(`#${id}`).removeClass('hidden'); }
        function closeModal(id) { $(`#${id}`).addClass('hidden'); }

        // ==========================================
        // LOGIKA MODAL CRUD OPD & PELAYANAN
        // ==========================================
